...
def tensor_to_cv2(tensor_img):
    """Converts a PyTorch tensor (C, H, W) [0,1] to OpenCV image (H, W, C) [0,255] BGR.
       Mimics the array conversion sequence used in original colie.py save step."""
    # Assume input tensor_img is [1, C, H, W] and normalized/clipped to [0, 1]
    # 1. Move dimensions C,H,W -> H,W,C
    img_np = torch.movedim(tensor_img, 1, -1) # Shape: [1, H, W, C]
    # 2. Remove batch dim, detach, cpu, numpy
    img_np = img_np[0].detach().cpu().numpy() # Shape: [H, W, C] (RGB, float [0,1])
    # 3. Scale by 255 and cast to uint8 (mimics colie.py save preparation)
    #    Note: astype(np.uint8) handles clipping implicitly here.
    img_np_uint8_rgb = (img_np * 255).astype(np.uint8) # Shape: [H, W, C] (RGB, uint8 [0,255])
    # 4. Convert final RGB uint8 array to BGR for OpenCV fusion steps
    ...
    for rgb_path, nir_path in tqdm(image_pairs, desc="Processing Images"):
        try:
            ...
            for epoch in range(opt.epochs):
                ...
                # Use the original sparsity loss from colie.py
            ...
            # Clip the result to [0, 1] instead of dividing by max
            img_rgb_fixed_tensor = torch.clamp(img_rgb_fixed_tensor, 0.0, 1.0)
