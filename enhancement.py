...
def process_image_pair(rgb_path: Path, nir_path: Path, out_path: Path, radius: int = 8, eps: float = 1e-3) -> None:
    """Process a single pair of RGB and NIR images."""
    ...
    if nir is None:
        print(f"Cannot load NIR image: {nir_path}")
        return

    # Perform fusion and denoising
    fused = fuse_rgb_nir(rgb, nir)
    deno = guided_denoise(fused, nir, radius=radius, eps=eps)
    
    # Save the result
    cv2.imwrite(str(out_path), deno)
    print(f"Processed: {rgb_path.name} + {nir_path.name} -> {out_path.name}")
...
def get_matching_pairs(rgb_dir: Path, nir_dir: Path) -> List[Tuple[Path, Path]]:
    ...
    rgb_files = {f.stem: f for f in rgb_dir.glob("*") if f.suffix.lower() in {'.png', '.jpg', '.jpeg', '.tiff'}}
    nir_files = {f.stem: f for f in nir_dir.glob("*") if f.suffix.lower() in {'.png', '.jpg', '.jpeg', '.tiff'}}
    
    # Find common filenames
    common_names = set(rgb_files.keys()) & set(nir_files.keys())
    return [(rgb_files[name], nir_files[name]) for name in common_names]

def main() -> None:
    ap = argparse.ArgumentParser(description="Batch RGB-NIR fusion with NIR-guided denoising")
    ap.add_argument("--rgb_dir", required=True, help="Directory containing RGB images")
    ap.add_argument("--nir_dir", required=True, help="Directory containing NIR images")
    ap.add_argument("--out_dir", default="output", help="Output directory for fused images")
    ap.add_argument("--radius", type=int, default=2, help="Guided-filter radius (pixels)")
    ...
    if not image_pairs:
        raise SystemExit("No matching RGB-NIR image pairs found")

    print(f"Found {len(image_pairs)} image pairs to process")

    # Process each pair with progress bar
    with tqdm(total=len(image_pairs), desc="Processing images", unit="pair") as pbar:
        for rgb_path, nir_path in image_pairs:
            out_path = out_dir / f"{rgb_path.name}"
            process_image_pair(rgb_path, nir_path, out_path, args.radius, args.eps)
            pbar.update(1)
